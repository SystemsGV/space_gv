...
function cls_emp_descripcion($id=0)
{
	...
	{
		$sql="select * from tbl_descripcion where id_cliente='".$id."'";
...
{
		$sql="insert tbl_descripcion(id_cliente,txt_descripcion,txt_info,int_stado) values('".
		$this->get_id_cliente()."','".
...
function actualiza()
{
		 $sql="update tbl_descripcion set 
...
function elimina()
{
	$sql="delete from tbl_descripcion where id_descripcion='".$this->get_id_descripcion()."';";
	conn_ejecuta($sql);			
}
