...
<script>
function eliminar(id,id0,img,idcl,idcat)
{
	var entrar = confirm("Esta seguro de eliminar este elemento?")
	if (entrar)
	{   
		location.href="proc_foto.php?op=3&idcl="+idcl+"&id="+id+"&id0="+id0+"&img="+img+"&idcat="+idcat;
	}
}
</script>
<style>
			  ...
			  <tr>
				<td height="4"><table width="98%" border="0" align="center" cellpadding="0" cellspacing="0">
				  ...
				  <tr>
					...
					<td height="30"><table width="100%" class="titu_secc"><tr><td align="left">&nbsp;Registro de Pagos - Paquete #<?=strtoupper($cliente->get_id_cliente())?></td><td align="right"><a href="inf_reqpago.php?idcat=<?=$idcat?>&idcl=<?=$_GET['idcl']?>" class="myButton4">< REGRESAR</a></td></tr></table></td>
				  </tr>
				  <tr>
					...
					<td width="464" align="left" valign="middle" bgcolor="#949494" class="titus_tablas">TITULO PAGO</td>
                    <td width="200" align="left" valign="middle" bgcolor="#949494" class="titus_tablas">MONTO PAGAGO</td>					  
                    <td width="200" align="left" valign="middle" bgcolor="#949494" class="titus_tablas">FECHA PAGO</td>
                    <td width="300" align="left" valign="middle" bgcolor="#949494" class="titus_tablas">REGISTRADO POR</td>
					<td width="211" align="left" valign="middle" bgcolor="#949494" class="titus_tablas"><div align="center">CONSTANCIA</div></td>
                    <?
					...
					<?
						$sql="select id_foto from tbl_foto where id_cliente='$_GET[idcl]' and id_foto0='$_GET[id0]' order by id_foto DESC";
					...
					<td height="95" class="texto_02" align="center"><? echo $contador?></td>
					...
					?>
                    <td align="left" class="texto_02"><?=$moneda;?>&nbsp;<strong><?=urldecode($datos->get_txt_titulo3())?></strong></td>
                    <td align="left" class="texto_02"><strong><?=urldecode($datos->get_int_stado())?></strong></td>
                    <td align="left" class="texto_02"><strong><?=urldecode($datos->get_int_retoque())?></strong></td>					  
                    
						  <?							
							$foto=UPLOAD_FOTOS.$datos->get_txt_foto();
						  ?>
					<td align="left" class="texto_02">
                    <?
                    if($datos->get_txt_titulo2()==""){
					?>
                    <div align="center"><a class="fancybox" href="<?=$foto?>"><img src="iconos/verfile.png" height="40" border="0" /></a></div>
                    <?
					}
					if($datos->get_txt_foto()==""){
					?>
                    <div align="center"><a class="fancybox" href="#inline<?=$datos->get_id_foto()?>" title="<?=urldecode($datos->get_txt_titulo())?>"><img src="iconos/verfile.png" height="40" border="0" /></a></div>
                    <?
					}
					?>
                    </td>
                    
                    <div id="inline<?=$datos->get_id_foto()?>" style="width:600px;display: none;">
                        <div style="padding:10px 10px 10px 10px;">
                        <p style="font-family:Verdana; font-size:13px;">
                        <?=nl2br(urldecode($datos->get_txt_titulo2()))?>
                        </p>
                        </div>
                    </div>
					...
					if($objcat0->getTipo()==0){
					?>
					<td align="left"><div align="center"><a href="frm_fotos.php?idcat=<?=$idcat?>&idcl=<?=$_GET[idcl]?>&id=<?=$datos->get_id_foto()?>&id0=<?=$_GET['id0']?>"><img src="iconos/ic_edit.png" border="0" alt="Editar registro" title="Editar registro" /></a>&nbsp;&nbsp;&nbsp;&nbsp;<a href="javascript:eliminar('<?=$datos->get_id_foto()?>','<?=$datos->get_id_foto0()?>','<?=$foto?>','<?=$_GET['idcl']?>','<?=$idcat?>')"><img src="iconos/ic_delete.png" border="0" alt="Eliminar registro" title="Eliminar registro" /></a></div></td>
                    <?
					}
					?>
                    <tr>
					...
					<table width="98%" border="0" align="center" cellpadding="0" cellspacing="0">
					  ...
					  <tr>
                        ...
                        <?
						  if($objcat0->getTipo()==0){
						  ?>
                        <a href="frm_fotos.php?idcat=<?=$idcat?>&idcl=<?=$_GET[idcl]?>&id0=<?=$_GET['id0']?>" class="myButton2">AGREGAR REGISTRO</a>
                        <?
                        }
						?>
                        </td>
