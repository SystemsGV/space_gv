...
if($_POST["cmd"]=="addcart"){
        ...
        if($_SESSION["sess_cart_sede"]["idsede"]){
            if($_SESSION["sess_cart_sede"]["idsede"]==$_POST["sede"]){
                ...
                for ($i=1; $i <=$_POST["txtcant"] ; $i++) {
                    ...
                    $data_descuento = Descuento::find_by_sql("SELECT descuento FROM descuento WHERE tipo=1 AND inicio <= '".$date."' AND fin >= '".$date."' AND st='1'");
	...
	if($_SESSION["sess_user"]["idusuario"]){
        ...
        if(!empty($_SESSION["sess_cart"])){
            $arr=array("msg"=>array("tipo"=>"alert","titulo"=>"Registro de Usuario","txtconfirm"=>"ok","txtcancel"=>"","content"=>$content,"urlconfirm"=>"?mod=login","urlcancel"=>"?mod=cart"));
        }else{
...
elseif ($_POST['cmd']=="save" && $_SESSION['sess_user'])
{
	...
	$cli = Cliente::find_by_sql("select id_usuarios,txt_apellido,txt_nombre,txt_email,txt_password from tbl_usuarios where txt_email='{$_POST["emailrecovery"]}'");
	...
	if($cli[0]->txt_email){
		...
		$body = "<p>Recibimos una solicitud de recordatorio de contrase&ntilde;a<br>Para ingresar a la tiendita deber&aacute; usar la siguiente: <strong>{$cli[0]->txt_password}</strong> <br><br><strong>La Granja Villa</strong></p>";  
