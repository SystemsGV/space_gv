   ...
   $idOrder = $_REQUEST["idf"];
    ...
    if(count($orderDetalle)){
        foreach($orderDetalle as $row){
            ...
            if($row->intboletoid){
                    $td.="<td style=\"border: 1px solid #dddddd;padding: 0.5rem;text-align: left;\">".$data_boleto->varboletotitulo."<BR>SEDE:".($order->intcartsede==1?"GRANJA VILLA SUR":"GRANJA VILLA NORTE")."</td>
    ...
    foreach($det as $k=>$v){
		...
		*(No incluye el Bungy, ni los juegos feriales o de destre-za donde se concursa por un premio). Para m&aacute;s detalles de los juegos visita: 
		...
		$terminosOther ='<table style="font-family:Arial; font-size:10px;" width="100%">
        <tr>
        <td width="100%" valign="top" style="font-family:Arial; font-size:10px;"><strong>BIENVENIDOS</strong> <br><br>
		Para que tu experiencia dentro del Festival del Terror sea segura y divertida, hemos preparado algunos tips para ti:<br>
		1. Los participantes deben presentar DNI. La edad m&iacute;nima de ingreso al festival es de 7 a&ntilde;os (sin las atracciones extremas). Solo las personas mayores de 10 a&ntilde;os, pueden ingresar a las atracciones extremas.<br>
		...
		9. Si las criaturas pose&iacute;das se escapan de sus zonas, escuchar&aacute;s alarmas por todo el festival. &iexcl;Corre y ref&uacute;giate en las zonas se&ntilde;alizadas!<br>
		...
		18. En el juego mec&aacute;nico &ldquo;Disco Nazca&rdquo;, no est&aacute; permitido el ingreso con sandalias que no tengan correas ajustables que pasen por detr&aacute;s del tobillo. Puedes comprar un sujetador en el kiosco si no traes un calzado ajustable.<br>
		...
		20. Si tienes entre 10 y 17 a&ntilde;os y quieres ingresar sin la compa&ntilde;&iacute;a de un adulto, el padre o apoderado deber&aacute; firmar un documento donde deje constancia que autoriza el ingreso del menor al festival.<br>
		...
		33. Si deseas m&aacute;s informaci&oacute;n, no dudes en comunicarte con nosotros al 717-7771 o escr&iacute;benos por cualquiera de nuestras redes sociales o p&aacute;gina web.<br>
		</td>
        </tr>
        </table>';
