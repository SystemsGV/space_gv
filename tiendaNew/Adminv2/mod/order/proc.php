...
switch ($_POST[act]) {
	...
	case 'upt':
		if($_POST[stado]=="2"){
			$upcart = Order::find($_POST[idf]);
			$upcart->intcartst=$_POST["stado"];
			$upcart->save();
		 	$post = Order::find_by_sql("select 
	ca.intCartId,
	ca.intcartst,
	ca.intCartSede,
	CASE ca.intCartSede  
	...
	where ca.intCartId='{$_POST[idf]}'");		
			...
			$mail->IsHTML(true);
			$membresiadet = Orderdet::find_by_sql("select * from orderdet where intCartId = ".$_POST['idf']." AND charCartdetTipo='M' limit 0,1");
		...
		cd.charCartdetDni,
		cd.decCartdetPu
		from orderdet cd left join boleto b
