...
$post = Cart::find('all',array(
  ...
  'conditions' => "varCartTitulo LIKE '%(PRESENCIAL)%'",
                ...
                <div class="box-body">
                  <table id="example1" class="table table-bordered table-hover display" width="100%" data-page-length="10">
					...
					foreach($post as $k=>$v){
						...
						$postdet2 = Cartdet::find_by_sql("SELECT COUNT(intCartdetId) AS seguro1 FROM cartdet WHERE intCartId = ".$v->intcartid." AND charCartdetTipo='T' AND varCartdetseguro=true");
						//var_dump($postdet);
						$membresia = "";
						...
						if($postdet2[0]->seguro1 > 0){
							$seguroadm= '<td class="text-center"><small class="label '.$estado[2].'">SÍ</small></td>';
						}else{
							$seguroadm= '<td class="text-center"><small class="label '.$estado[3].'">NO</small></td>';
						}
					?>
                      <tr>
                        <td><?php echo $i; ?></td>
