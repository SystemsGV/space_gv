   ...
   $idOrder = $_REQUEST["idf"];
    ...
    if(count($orderDetalle)){
        foreach($orderDetalle as $row){
			...
			$cartDetalle->varcartdetseguro = $row->varcartdetseguro;
    ...
    $newtxt  = array($order->varcartcreserva, ucwords($cliente->sclieapel." ".$cliente->scliename),$cliente->charclientedni,date("d-m-Y",strtotime($order->datecartfreg)),count($orderDetalle),number_format($order->deccarttotal,2,".",""),$td,number_format($order->deccartstotal,2,".",""),number_format($order->deccartigv,2,".",""),number_format($order->deccarttotal,2,".",""));
