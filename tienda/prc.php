<?php
    ...
    $path="http://www.lagranjavilla.com/tiendaNew/";
    $host="http://www.lagranjavilla.com/tiendaNew/";
    $ruta="/home3/lagranja/public_html/tiendaNew/";
    $arr = array();
    $sede[1]="GRANJA VILLA SUR";
    $sede[2]="GRANJA VILLA NORTE";
    $sede[6]="GRANJA VILLA SUR";
    $sede[11]="GRANJA VILLA SUR";
    $sede[13]="GRANJA VILLA SUR";
    $seguro=$_POST["Seguro2"];
    $accesorio=$_POST["txt_acc"];
    ...
    if($_POST["cmd"]=="addcart"){
        ...
        if($_POST["txtcant"]){
            if($_SESSION["sess_cart_sede"]["idsede"]){
                //de la 2 entrada a más se calcula aca 
                if($_SESSION["sess_cart_sede"]["idsede"]==$_POST["sede"]) {
                    ...
                    for ($i=1; $i <=$_POST["txtcant"] ; $i++) {
                        ...
                        $data_descuento = Descuento::find_by_sql("SELECT descuento FROM descuento WHERE tipo='".$sedet."' AND inicio<='".$date."' AND fin>='".$date."' AND st='1'");
                        if(count($data_descuento)){
                            $precio_boleto = $data_descuento[0]->descuento;
                        }
    					if($_POST["seguro"]=='1'){
    						$precio_boleto = $precio_boleto + $seguro + $accesorio;
    						$segr=$_POST["seguro"];
    					}else{
    						$segr='0';
    						$precio_boleto = $precio_boleto + $accesorio;
    					}
    
                        $arrSess=array("titulo"=>$post->varboletotitulo,"img"=>$post->varboletoimg,"txtfec"=>$_POST["txtfec"],"txttotal"=>$precio_boleto,"sede"=>$_POST["sede"],"txtcant"=>$_POST["txtcant"],"sel"=>$_POST["sel"],"tipo"=>"T","seguro"=>$segr);
                ...
                //cuando ingresa se calcula aca la 1ra entrada
                $_SESSION["sess_cart_sede"]["idsede"]=$_POST["sede"];
                $_SESSION["sess_cart_sede"]["nombre"]=$sede[$_POST["sede"]];
                for ($i=1; $i <=$_POST["txtcant"] ; $i++) {
    				$sedet = $_POST["sede"];
        ...
        if(!empty($_SESSION["sess_cart"])){
            ...
            $total  = number_format(array_sum(array_column($_SESSION["sess_cart"],'txttotal')), 2,'.','');
            $igv    = number_format(0.18 * $total, 2,'.','');
            $stotal = number_format($total-$igv, 2,'.','');
        	...
        	{
    			if(!isset($_SESSION["sess_cart_sede"]["idsede"])){
                ...
                if(count($_SESSION["sess_cart"])){
                    foreach ($_SESSION["sess_cart"] as $key => $value) {
                        ...
                        $postdet->deccartdetpu=$value["txttotal"];
                        $postdet->intcartdetcant=1;
                        $postdet->deccartdetstotal=$value["txttotal"] * 1;
    					...
    					$postdet->varcartdetseguro=$value["seguro"];
                        $postdet->charcartdettipo=$value["tipo"];
                        $postdet->intcartdetst="1";
                        $postdet->save();
                        $total = $total + $value["txttotal"];
                    }
                }
        		...
        		if(count($_SESSION["sess_cart"])){
                    foreach ($_SESSION["sess_cart"] as $key => $value)
                    {
                        ...
                        $td.="<tr>
                            ...
                            <td style=\"border: 1px solid #dddddd;padding: 0.5rem;text-align:right;\">S/. ".number_format($value['txttotal'],2,".","")."</td>
                        </tr>";
                    }
    		    }
